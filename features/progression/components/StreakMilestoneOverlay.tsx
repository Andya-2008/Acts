...
type StreakMilestoneOverlayProps = {
  days: number;
  title: string;
  message: string;
  onClose: () => void;
...
/** Full-screen streak milestone (tap anywhere to dismiss). */
export function StreakMilestoneOverlay({ days, title, message, onClose }: StreakMilestoneOverlayProps) {
  ...
  const runEnter = () => {
    ...
    Animated.parallel([
      ...
      Animated.sequence([
        Animated.spring(numberScale, { toValue: 1, friction: 6, tension: 120, useNativeDriver: true }),
        Animated.timing(numberScale, { toValue: 0.94, duration: 120, useNativeDriver: true }),
        Animated.spring(numberScale, { toValue: 1, friction: 8, tension: 140, useNativeDriver: true }),
      ]),
    ]).start();
        ...
        <Animated.View
          ...
          <View
            ...
            <Animated.View style={{ transform: [{ scale: numberScale }] }}>
              <Ionicons name="flame" size={52} color={accent} />
            </Animated.View>
          </View>

          <Animated.View style={{ transform: [{ scale: numberScale }], marginTop: 20 }}>
            <AppText
              variant="title"
              paletteColor={false}
          ...
          <AppText
            variant="caption"
            paletteColor={false}
            className="mt-4 text-center font-bold uppercase tracking-[0.2em]"
